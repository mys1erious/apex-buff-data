...
def get_weapon_data_from_weapon(weapon):
    str_fields = ['name', 'weapon_type', 'mag_size', 'projectile_speed']
    weapon_img_path = f'{WEAPON_IMAGES_DIR}/{weapon["name"]}.png'
...
def get_attachments_data_from_weapon(weapon):
    ...
    for ind, attachment in enumerate(attachments):
        attachments[ind] = slugify(attachment)
...
def post_weapon(weapon, url):
    ...
    if post_weapon_response.ok:
        ...
        for attachment in attachments_data:
            post_weapon_attachment_response = requests.post(
                url=weapon_url+'/attachments/',
...
def post_weapons(weapons, url):
    for weapon_name in weapons:
        ...
        post_weapon(data, url)


if __name__ == '__main__':
    url = HEROKU_DEV_BASE_API_URL

    weapons_data = load_json_data(WEAPONS_JSON)

    # for weapon_name in weapons_data:
    #     weapon = weapons_data[weapon_name]
    #     data = {
    #         'damage_body': weapon['body_damage'],
    #         'damage_head': weapon['head_damage'],
    #         'damage_legs': weapon['legs_damage']
    #     }
    #
    #     print(data)
